...
public class ExtensionVariableISView extends BaseUIView {
	...
	public void processMessage(UIViewContext context) {
		...
		this.processLayoutControls(context, items);
	}
	...
	protected String formatContent(String html, UIViewContext context) {
		for (String key : context.getClipboard().keySet()) {
			html = CoreStringUtil.replaceVar(html, key, this.getString(context.getClipboard(), key));
		}
		return html;
	}
